...
public class CommandMovecraft implements TabExecutor{
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
        if(!sender.hasPermission("movecraft.reload")&&!sender.hasPermission("movecraft.debug")){
            sender.sendMessage(ChatColor.RED+"Insufficient permission!");
            return true;
        }
        if(args.length<1){
            sender.sendMessage("Usage: /movecraft reload|debug [on|off]");
            return true;
        }
        if(args[0].equals("reload")){
            if(!sender.hasPermission("movecraft.reload")){
                sender.sendMessage(ChatColor.RED+"Insufficient permission!");
                return true;
            }
            if(args.length!=1){
                sender.sendMessage("Usage: /movecraft reload");
                return true;
            }
            plugin.reloadConfig();
            plugin.reload();
        ...
        if(args[0].equals("debug")){
            if(!sender.hasPermission("movecraft.debug")){
                sender.sendMessage(ChatColor.RED+"Insufficient permission!");
                return true;
            }
            if(args.length>2){
                sender.sendMessage("Usage: /movecraft debug [on|off]");
                return true;
            }
            if(args.length==1){
