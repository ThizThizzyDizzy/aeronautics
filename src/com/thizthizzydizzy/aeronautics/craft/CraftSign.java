...
public abstract class CraftSign{
    ...
    public static void init(Aeronautics aeronautics){
        signs.clear();
        addSign(new CraftSign("aeronautics:pilot_craft"){
            @Override
            public boolean matches(Craft craft, Sign sign){
                String name = sign.getLine(0);
                for(CraftType type : aeronautics.craftTypes){
            ...
            public void click(Craft craft, Sign sign, PlayerInteractEvent event){
                ...
                if(craft==null){
                    //pilot new craft
                    aeronautics.debug(event.getPlayer(), "Attempting to pilot "+name);
                    Craft newCraft = aeronautics.detect(piloting, event.getPlayer(), sign.getBlock());
                    if(newCraft!=null&&newCraft.canPilot(event.getPlayer())){
                        newCraft.addPilot(event.getPlayer());
                        aeronautics.addCraft(newCraft);
                    }
                }else{
                    if(craft.type==piloting){
                        if(craft.canPilot(event.getPlayer())){
                            if(craft.isPilot(event.getPlayer())){
                                aeronautics.debug(event.getPlayer(), "You are already piloting this craft!");
                            }else{
                                craft.addPilot(event.getPlayer());
                                aeronautics.debug(event.getPlayer(), "You are now co-piloting this craft");
                            }
                        }
                    }else{
