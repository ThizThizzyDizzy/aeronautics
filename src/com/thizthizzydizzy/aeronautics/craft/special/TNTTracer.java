    ...
    public void tick(CraftSpecial special){
        ...
        for(Iterator<TNTPrimed> it = tnts.keySet().iterator(); it.hasNext();){
            ...
            if(tnt.getFuseTicks()<=0){
                if(explosionBlock!=null)createGhostBlock(special.getCraft().aeronautics, tnt.getLocation(), explosionBlock, explosionTime);
                it.remove();
                continue;
            }
            ...
            if(tracerBlock!=null){
                if(vel>squareVelocityThreshold){
                    ...
                    if(nextTracer<=0){
                        createGhostBlock(special.getCraft().aeronautics, tnt.getLocation(), tracerBlock, tracerTime);
                    }
